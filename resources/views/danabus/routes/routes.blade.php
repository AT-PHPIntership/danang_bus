  ...
  @section('content')
    <div class="section secondary-section " id="portfolio">
      <div class="container">
        <div class=" title">
          <h1> {!! trans('routes.list_route') !!} </h1>
        </div>
        <div class="container">    
          <div class="row">
            <div class="on-city" > {!! trans('routes.supurban') !!}</div>
            @foreach ($municipal as $value)
           
            <div class="col-sm-4">
              <div class="panel panel-primary">
                <div class="panel-heading">
                  <a  href="/tuyen/{{$value ->id}}"> {{trans('routes.routes')}} : {{ $value ->name }}</a>
                </div>
              </div>
            </div>
            @endforeach
          ...
          <div class="row">
            ...
            <div class="col-sm-4"> 
              <div class="panel panel-primary">
                <div class="panel-heading"><a  href="/tuyen/{{$value ->id}}">{{trans('routes.routes')}}: {{ $value ->name }}</a></div>
              </div>
            </div>
            @endforeach
